...
public interface PacketRepository extends JpaRepository<OutPacket, Long> {
  ...
  @Query(value =
      "select distinct P.ID, P.USER USER, P.BIND_ID CLIENT_ID, P.CLIENT_VER_ID PUSH_VER_ID," +
          " P.COMMENT COMMENT, TC.VALUE_COLUMN CLASS_ID, TN.VALUE_COLUMN OBJECT_NAME, P.SENT SENT,"
          +
          "P.DATE PUSH_DATE, T.VALUE_COLUMN OBJECT_ID, TA.VALUE_COLUMN ACTION, TR.VALUE_COLUMN REMOVED"
          +
          ...
          +
          "inner join T_ACTION TA on T.CLIENT_VER_ID = TA.CLIENT_VER_ID and T.KEY_COLUMN = TA.KEY_COLUMN "
          +
          "inner join T_REMOVED TR on TA.CLIENT_VER_ID = TR.CLIENT_VER_ID and TA.KEY_COLUMN = TR.KEY_COLUMN "
          +
